...
def test_patch_200_success_default():
    """Tests a PATCH request to the /v2/nvcf/authorizations/functions/{functionId}/remove endpoint.
    ...
    response = client.nvcf.authorizations.functions.remove.patch(
...
async def test_await_patch_200_success_default():
    ...
    response = await client.nvcf.authorizations.functions.remove.patch(
        authorized_party={"nca_id": "string"},
        function_id="3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a",
    )
    try:
