...
def test_get_200_generated_success():
    ...
    Response : models.PexecStatusGetResponse
    ...
    response = client.pexec.status.get(
        request_id="3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a"
    )
    try:
        pydantic.TypeAdapter(models.PexecStatusGetResponse).validate_python(response)
        is_json = True
...
async def test_await_get_200_generated_success():
    ...
    response = await client.pexec.status.get(
        request_id="3e4666bf-d5e5-4aa7-b8ce-cefe41c7568a"
    )
    try:
        pydantic.TypeAdapter(models.PexecStatusGetResponse).validate_python(response)
